...
pro TVCurtain_mao, Data,  timeArr,  ZArr,                          $
...
;   if ( N_Elements( LonArr ) eq 0 ) then LonArr = FindGen( SData[0] )
   if ( N_Elements( timeArr ) eq 0 ) then timeArr = FindGen( SData[0] )
...
;         Position=NewPosition, charsize=plotcsfac*csfac, XMinor=0,      $
;         XTicks=nXTick,      _EXTRA=e

;      Plot, XArr, ZArr, $
;         /NoData,              /NoErase,                 Color=MColor,  $
;         XRange=XRange,        YRange=YRange,            XStyle=1,      $
;         YStyle=YStyle,        XTitle=XTitle,            YTitle=YTitle, $
;         Position=NewPosition, XMinor=2,      $
;         XTicks=nXTick,      _EXTRA=e
...
;         XRange=XRange,        YRange=YRange,            XStyle=1,      $
;         YStyle=YStyle,        XTitle=XTitle,            YTitle=YTitle, $
;         Position=NewPosition,    XMinor=1,      $
;	 XTicks=nXTick,      XTICKUNITS='Time',   XTICKFORMAT='LABEL_DATE',_EXTRA=e
;      !x.tickname=TickStore

;I copy this part from Barkley and modify it.
; ...next generate the time axis...
DateFmt      = '%d/%n'  ; Format of time-axis %m = jul $n = 07

nTimeSteps=nXTick
ndt      = nTimeSteps 
dummy    = LABEL_DATE( DATE_FORMAT=DateFmt )
months   = TIMEGEN( ndt ,  UNITS='DAYS' , START=timearr[0] )
null     = FINDGEN( ndt )
NoXticks = ndt 
;NoYticks = ( YR[1]  - YR[0] )  / dHt


PLOT, months , ZArr , /NODATA, COLOR=pCol , $
    XTICKUNITS='Time' , XTICKFORMAT='LABEL_DATE',XTICKS=NoXticks   , $
    YRANGE=YRange , YSTYLE=ystyle , YTITLE=yInfo,  YTICKS=NoYticks , $ 
    XMINOR=1 , YMINOR=1      , $
    XTICKLEN=-0.03,YTICKLEN=-0.02, THICK=LTK ,$ 
    POSITION=NewPosition  , XSTYLE=8
...
;         Color=MColor, XTickName=XticknameTime,/Norm, XMinor=1,         $
